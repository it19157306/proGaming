  ...
  <div class="main">
    <div class="row">
      ...
      <div class="col-xl-10 col-lg-10 col-md-9 main-screen">
        <!-- Page heading -->
        <h3 class="sect-header">All Categories</h3>
        <!-- Page navigation -->
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="${pageContext.request.contextPath}/UserController?action=Dashboard">Home</a></li>
            <li class="breadcrumb-item"><a href="${pageContext.request.contextPath}/CategoryController">Categories</a></li>
        ...
        <div class="admin-table all-cat align-center">
            ...
            <table id="example" class="table table-striped table-bordered table-responsive-lg" style="width:100%">
                ...
                <c:forEach items = "${list}" var = "cat">
                    <tr>
                        <td>${cat.cat_id}</td>
                        <td><div class= "cat-icon">${cat.cat_icon}</div></td>
                        <td>${cat.name}</td>           
                        <td> <a href="${pageContext.request.contextPath}/CategoryController?action=EDIT&id=${cat.cat_id}">Edit</a>  |  <button class="del" onClick ="deleteUser(${cat.cat_id});"> Delete</button></td>
                    </tr>
...
//Prompts confirmation message before deleting
	function deleteUser(id){
		if(confirm("Are you sure you want to delete ?"))
			{
				window.location.href = "<%=request.getContextPath()%>/CategoryController?action=DELETE&id="+id;
			}
	}
	...
	if (action == null && id == null ){
		window.location.href = "<%=request.getContextPath()%>/CategoryController?action=success"; /* This redirects the link back to the server only once and displays the table to prevent adding multiple records to the db on reloading the page */
	}
 ...
 <script type="text/javascript" >
	 ...
	 var sublink = document.getElementById('all-cat');
